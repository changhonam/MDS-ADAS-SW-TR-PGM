...
## 2.4 Operating Mode
Operating Mode는 현재 프로세서가 어떤 구너한을 가지고 어떤 종류의 작업을 하고 있는지를 나타낸다.
- User Mode
    - User task 또는 어플리케이션을 수행할 때의 프로세서의 동작모드 (비유 : Linux의 user 권한)
    - ARM 프로세서의 동작 모드 중 유이랗게 <u>Un-privileged 모드</u>
